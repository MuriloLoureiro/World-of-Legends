...
if (oldLevel != level) {
	...
	f(Pokemon * pokemon = getHisPokemon()) {
		pokemon->setLevel(getLevel());
		setPokemonHealthMax(pokemon->getMaxHealth());
		setPokemonHealth(pokemon->getMaxHealth());
		pokemon->changeHealth(pokemon->getMaxHealth());
...
if (isDualWielding()) {
	double multiplier = 1000.0 / static_cast<double>(g_config.getNumber(ConfigManager::DUAL_WIELDING_SPEED_RATE));
...
if ((getHisPokemon() && attacker->getAttackedCreature() != this) || damage <= 0) {
	damage = 0;
	return BLOCK_ARMOR;
}
...
if ((slotPosition & SLOTP_LEFT_THREE) || (slotPosition & SLOTP_LEFT_TWO) ||
	(slotPosition & SLOTP_LEFT_ONE) || (slotPosition & SLOTP_HEAD) ||
...
case CONST_SLOT_ARMOR: {
	...
	break;
case CONST_SLOT_LEFT_ONE: {
	if (slotPosition & SLOTP_LEFT_ONE) {
		ret = RETURNVALUE_NOERROR;
	}
	break;
case CONST_SLOT_LEFT_TWO: {
	if (slotPosition & SLOTP_LEFT_TWO) {
		ret = RETURNVALUE_NOERROR;
	}
	break;
case CONST_SLOT_LEFT_THREE: {
	if (slotPosition & SLOTP_LEFT_THREE) {
		ret = RETURNVALUE_NOERROR;
	}
	break;
}
