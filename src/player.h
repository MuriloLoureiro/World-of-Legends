...
public:
	...
	//POKEWOL
	void setPokewolHealthMax(uint32_t health) {
		pokewolHealthMax = health;
	}
	void setPokewolHealth(uint32_t health) {
		pokewolHealth = health;
	}
	uint32_t getPokemonHealth() const {
		return pokewolHealth;
	}
	uint32_t getPokemonHealthMax() const {
		return pokewolHealthMax;
	}
	...
	uint8_t getPokemonCapacity() const {
		...
		else if (hasFlag(PlayerFlag_HasInfinitePokewolCapacity)) {
	...
	uint8_t getFreePokemonCapacity() const {
		if (hasFlag(PlayerFlag_CannotPickupItem)) {
			...
			return std::max<uint8_t>(0, 2 - inventoryPokemonCount);
		}
	}
	...
	BlockType_t getLastAttackBlockTypegetLastAttackBlockType() const {
	...
	sendCreatureGender(const Creature* creature) const {
		if (client) {
...
private:
	...
	uint32_t pokewolHealth = 0;
	uint32_t pokewolHealthMax = 0;
